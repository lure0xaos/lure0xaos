...
public enum Aliases {
    ...
    public static String toAlias(String input) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            int idx = Arrays.binarySearch(REPLACE, chr);
            if (idx < 0)
                sb.append(chr.toLowerCase(Locale.ENGLISH));
            else
                sb.append(REPLACEMENT[idx].toLowerCase(Locale.ENGLISH));
        }
        return sb.toString();
    }
//    public static String toAlias2(String input) {
//        if (input == null || input.isEmpty())
//            return "";
//        String result = input.replace(SPACE, UNDERSCORE);
//        for (int i = 0; i < REPLACE.length; i++) {
//            result = result.replace(REPLACE[i], REPLACEMENT[i]);
//        }
